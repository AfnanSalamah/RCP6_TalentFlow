...
// Retry idempotent reads on transient failures (network error / 5xx). Mutating
// verbs (POST/PUT/PATCH/DELETE) are never retried to avoid duplicate writes.
const MAX_RETRIES = 2;
...
async function request(path, options = {}, tokenType = "applicant") {
  ...
  for (let attempt = 0; attempt <= (retryable ? MAX_RETRIES : 0); attempt++) {
    ...
    try {
      ...
      // Connection refused / offline — retry reads, then surface a clear error.
      lastErr = new Error("Network error — could not reach the server.");
      if (retryable && attempt < MAX_RETRIES) { await sleep(300 * (attempt + 1)); continue; }
      throw lastErr;
    }
    ...
    // Retry server errors on reads only.
